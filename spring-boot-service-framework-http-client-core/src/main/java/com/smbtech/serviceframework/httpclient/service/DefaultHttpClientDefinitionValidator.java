...
public final class DefaultHttpClientDefinitionValidator implements HttpClientDefinitionValidator {
    ...
    public void validate(HttpClientDefinition definition) {
        ...
        if (definition.beanName().isBlank()) {
            throw new HttpClientConfigurationException("HTTP client beanName is required for: " + definition.name());
        }
        if (definition.baseUrl() == null) {
            throw new HttpClientConfigurationException("baseUrl is required for HTTP client: " + definition.name());
        }
    ...
    private void validateAuthentication(HttpClientDefinition definition) {
        ...
        if ((definition.authenticationType() == AuthenticationType.CLIENT_CREDENTIALS
                ...
                && definition.credentialTokenRequestorId().isBlank()) {
            throw new HttpClientConfigurationException(
                    "credentialTokenRequestorId is required for HTTP client: " + definition.name()
            );
        }
    }
}
