...
class LoggingAutoConfigurationTest {
    ...
    void registersTransactionFilterByDefault() {
        webContextRunner.run(context -> assertThat(context)
                ...
                .hasSingleBean(TransactionalIdFilter.class));
    }
    ...
    void canDisableTransactionFilter() {
        webContextRunner
                ...
                .run(context -> assertThat(context).doesNotHaveBean(TransactionalIdFilter.class));
    }

    @Test
    void backsOffWhenConsumerProvidesPorts() {
        StructuredLoggerFactory customFactory = source -> null;
        CorrelationContext customContext = new CorrelationContext() {
            @Override
        ...
        webContextRunner
                ...
                .run(context -> {
                    assertThat(context).getBean(StructuredLoggerFactory.class)
                            .isSameAs(customFactory);
                    assertThat(context).getBean(CorrelationContext.class)
                            .isSameAs(customContext);
                    assertThat(context).hasSingleBean(StructuredLoggerFactory.class)
                            .hasSingleBean(CorrelationContext.class);
                });
    }
    ...
    void doesNotCreateServletAdapterOutsideServletApplication() {
        contextRunner.run(context -> assertThat(context)
                .hasSingleBean(StructuredLoggerFactory.class)
                .hasSingleBean(CorrelationContext.class)
                .doesNotHaveBean(TransactionalIdFilter.class));
    }
