...
class MockRestClientRequestMapperTest {
    ...
    void mapsSpringHttpRequestToNeutralMockRequest() {
        var request = mapper.toMockRequest(
                httpRequest(
                        HttpMethod.POST,
                        URI.create("https://api.example.test/v1/payments?id=123&id=456"),
                        Map.of("X-Mock-Key", List.of("payments-success"), "X-Trace-Id", List.of("trace-1"))
                ),
        ...
        assertThat(new String(request.body(), StandardCharsets.UTF_8)).isEqualTo("{\"amount\":100}");
    ...
    void supportsCustomMockKeyHeader() {
        ...
        var request = customMapper.toMockRequest(
                httpRequest(
                        HttpMethod.GET,
