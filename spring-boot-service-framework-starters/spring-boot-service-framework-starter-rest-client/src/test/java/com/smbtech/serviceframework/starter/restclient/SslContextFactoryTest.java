...
class SslContextFactoryTest {
    ...
    void buildsClientSslContextFromConfiguredTrustStoreAndKeyStore() throws Exception {
        ...
        new ApplicationContextRunner()
                ...
                .withPropertyValues(
                        ...
                        "smbtech.rest-clients.authentication.key-stores.payments-trust.location=file:" + keyStore,
                        ...
                        "smbtech.rest-clients.authentication.key-stores.payments-client-cert.location=file:" + keyStore,
                ...
                .run(context -> {
                    HttpClientDefinition definition = context.getBean(HttpClientCatalog.class)
                            .requireByName("payments");

                    SSLContext sslContext = context.getBean(SslContextFactory.class)
                            .build(definition, null);

                    assertThat(definition.apache().ssl().enabled()).isTrue();
                    assertThat(definition.apache().ssl().trustStoreId()).isEqualTo("payments-trust");
                    assertThat(definition.apache().ssl().keyStoreId()).isEqualTo("payments-client-cert");
    ...
    @Test
    void buildsClientSslContextFromBase64TrustStoreKeyStoreAndCredentialPasswords() throws Exception {
        String keyStoreBase64 = Base64.getEncoder().encodeToString(Files.readAllBytes(createKeyStore("inline-client")));
        String passwordBase64 = Base64.getEncoder().encodeToString("changeit".getBytes(StandardCharsets.UTF_8));
        ...
        new ApplicationContextRunner()
                ...
                .withPropertyValues(
                        ...
                        "smbtech.rest-clients.clients.payments.apache.ssl.trust-store-id=payments-trust",
                        "smbtech.rest-clients.clients.payments.apache.ssl.key-store-id=payments-client-cert",
                        "smbtech.rest-clients.authentication.credentials.store-password.base64=" + passwordBase64,
                        "smbtech.rest-clients.authentication.credentials.key-password.base64=" + passwordBase64,
                        "smbtech.rest-clients.authentication.key-stores.payments-trust.base64=" + keyStoreBase64,
                        "smbtech.rest-clients.authentication.key-stores.payments-trust.type=PKCS12",
                        "smbtech.rest-clients.authentication.key-stores.payments-trust.password-ref=store-password",
                        "smbtech.rest-clients.authentication.key-stores.payments-client-cert.base64=" + keyStoreBase64,
                        "smbtech.rest-clients.authentication.key-stores.payments-client-cert.type=PKCS12",
                        "smbtech.rest-clients.authentication.key-stores.payments-client-cert.password-ref=store-password",
    ...
    void trustStoreDoesNotRequireKeyAlias() throws Exception {
        ...
        new ApplicationContextRunner()
                ...
                .withPropertyValues(
                        ...
                        "smbtech.rest-clients.authentication.credentials.store-password.value=changeit",
                        "smbtech.rest-clients.authentication.key-stores.audit-trust.location=file:" + trustStore,
                ...
                .run(context -> {
                    ...
                    assertThat(definition.apache().ssl().trustStoreId()).isEqualTo("audit-trust");
                    assertThat(sslContext).isNotNull();
                    assertThat(sslContext.getProtocol()).startsWith("TLS");
                });
    }
